...
categoryNotifications.onTopicPost = async function (topic) {
	...
	settings['notification-trigger'] = settings['notification-trigger'] || 'topics-only';
	
	winston.verbose(`[category-notifications] Topic created, trigger setting: ${settings['notification-trigger']}`);
...
categoryNotifications.onTopicReply = async function (post) {
	...
	winston.verbose(`[category-notifications] Reply posted, trigger setting: ${settings['notification-trigger']}`);
	
	// Solo notificar respuestas si está configurado como 'both'
	if (settings['notification-trigger'] === 'topics-only') {
		winston.verbose(`[category-notifications] Skipping reply notification (topics-only mode)`);
		return;
	}
